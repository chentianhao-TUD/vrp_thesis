...
path = 'instances/10/10-2-7-100.xml'
instances = [0, 1, 2, 3, 4]


def build_cp_model(data, instances):
    ...
    model.Add(start_service[data["end_depot_vehicles"]] * len(data["robots"]) >=
              sum(w[i, j] * data["distances"][i, j] for i in data["all_nodes"][:-1] for j in data["all_nodes"][1:]) +
              sum(service_time[h] for h in data["arrival_nodes"]))  # 31

    model.Add(start_service[data["end_depot_vehicles"]] * len(data["vehicles"]) >=
              sum(x[i, j] * data["distances"][i, j] for i in data["all_nodes"][:-1] for j in data["all_nodes"][1:]))  # 32
    ...
    for j in data["all_nodes"][1:]:
        model.Add(start_service[j] >= start_service[data["start_depot_vehicles"]] +
                  sum((data["distances"][data["start_depot_vehicles"], i] + data["distances"][i, j]) * x[i, j]
                      for i in data["all_nodes"][:-1]))  # 34

    for j in data["arrival_nodes"]:
        model.Add(sum(w[i, j] for i in data["all_nodes"][:-1]) >= 1)  # 35

    for i in data["arrival_nodes"]:
        model.Add(sum(w[i, j] for j in data["all_but_corresponding_nodes"][i]) <= data["transport_capacity"] - 1)  # 36

    for j in data["departure_nodes"]:
        model.Add(sum(w[i, j] for i in data["all_but_corresponding_nodes"][j]) <= data["transport_capacity"] - 1)  # 37

    for i in data["departure_nodes"]:
        model.Add(sum(w[i, j] for j in data["all_nodes"][1:]) >= 1)  # 38
...
def main(path, instances):
    for i in instances:
        data = generate_input_data(path, i)
        data["distances"].update((key, round(value)) for key, value in data["distances"].items())
        build_cp_model(data, i)
