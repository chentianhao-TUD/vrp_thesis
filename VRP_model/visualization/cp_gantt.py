...
def get_distance_matrix(xml_path, INSTANCE, n_START_DEPOTS_ROBOTS):
...
def get_vars(SOLUTION_FILE_NAME, END_DEPOT, n_start_depots_robots, N_FIELDS):
...
def main(N_FIELDS, N_VEHICLES, N_ROBOTS):
    ...
    for INSTANCE in range(5):
        distances = get_distance_matrix(xml_path, INSTANCE, n_START_DEPOTS_ROBOTS)
        SOLUTION_FILE_NAME = "Results_CP_VRP_" + CONFIGURATION + "_" + str(INSTANCE) + ".csv"  # Results_CP_VRP_5-4-11-100_4
        plt.figure(2)
        fig, gnt = plt.subplots()
        x, w, service_time, start_service, v = get_vars(SOLUTION_FILE_NAME, END_DEPOT, n_start_depots_robots, N_FIELDS)
        ...
        plt.savefig(f"CP_VRP_gantt_{CONFIGURATION}_{INSTANCE}.png")
        plt.close()
